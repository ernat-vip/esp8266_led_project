...
var speedRange = document.getElementsByTagName('input')[1];
...
try{
	...
	speedRange.addEventListener('touchstart', speedclickdown);
	speedRange.addEventListener('touchend', speedclickup);
}
catch(e){ 
	inputRange.addEventListener('mousedown', clickdown);
	inputRange.addEventListener('mouseup', clickup);
	speedRange.addEventListener('mousedown', speedclickdown);
	speedRange.addEventListener('mouseup', speedclickup);
}

function clickdown(){rangeValue.style.opacity = '1';}
function clickup(){rangeValue.style.opacity = '0';}
function speedclickdown(){speedValue.style.opacity = '1';}
function speedclickup(){speedValue.style.opacity = '0';}
...
function posSpeedValue(){
		//ширира дорожки - ширина элемента
		var widthBlock = (speed.clientWidth - speedValue.clientWidth) / 100;
	
		speedValue.style.transform = 'translateX(' + range.value  * widthBlock + 'px)';
		speedValue.innerText = range.value;
	
		speed.oninput = function(){
			speedValue.innerText = parseInt(this.value);
			speedValue.style.transform = 'translateX(' + (this.value * widthBlock) + 'px)';
	
			var send = 'S_' + this.value;
			console.log(send);
			Socket.send(send);
